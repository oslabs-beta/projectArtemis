...
const QuerySuccessFailureGraph = (props: Props) => {
    ...
    datasets: [
      {
        ...
        data: aggregateMetrics
          ? [aggregateMetrics.queryFrequency, aggregateMetrics.errorFrequency]
          : [],
      },
    ],
  };
  const options = {
    ...
    title: {
      display: true,
      text: `Successful Queries : ${
        aggregateMetrics
          ? (aggregateMetrics.queryFrequency / aggregateMetrics.queryTotal *
            100).toFixed(3)
          : 0
      }%           Unsuccessful Queries: ${
        aggregateMetrics
          ? (aggregateMetrics.errorFrequency / aggregateMetrics.queryTotal *
            100).toFixed(3)
          : 0
      }%`,
    },
